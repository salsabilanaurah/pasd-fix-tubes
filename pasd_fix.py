...
def show_ml_prediction():
    ...
    new_data = [[price, 0, combination, dry, normal, oily]]
    ...
    for i in range(k):
        index = indices[0][i]
        product = df.iloc[X_train.index[index], :]
        if product['Label'] == selected_label and product[selected_skin_type] == 1:
            product_found = True
            st.write(f"{count}. {product['Brand']} - {product['Name']}")
            count += 1
            if count > 5:  # Jumlah rekomendasi yang ingin ditampilkan
                break
